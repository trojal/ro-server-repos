...
struct mob_data* mob_spawn_dataset(struct spawn_data *data)
{
	...
	md->level = data->level;
	md->minlevel = data->minlevel;
	md->maxlevel = data->maxlevel;
 ...
 *------------------------------------------*/
int mob_once_spawn(struct map_session_data* sd, int m, short x, short y, const char* mobname, int class_, int amount, const char* event, int minlv, int maxlv)
{
 ...
 *------------------------------------------*/
int mob_once_spawn_area(struct map_session_data* sd,int m,int x0,int y0,int x1,int y1,const char* mobname,int class_,int amount,const char* event, int minlv, int maxlv)
{
	int i,max,id=0;
	int lx=-1,ly=-1;
	...
	for( i = 0; i < amount; i++)
	{
		...
		id = mob_once_spawn(sd,m,x,y,mobname,class_,1,event,minlv,maxlv);
	}
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	for(i = 0; i < DAMAGELOG_SIZE && md->dmglog[i].id; i++)
	{
		...
		if (map[m].flag.nobaseexp || !md->db->base_exp)
			...
			base_exp = (unsigned int)cap_value(((md->db->base_exp/99.0)*md->level) * per * bonus/100. * map[m].bexp/100., 1, UINT_MAX);
		
		if (map[m].flag.nojobexp || !md->db->job_exp || md->dmglog[i].flag) //Homun earned job-exp is always lost.
			job_exp = 0; 
		else
			job_exp = (unsigned int)cap_value(((md->db->job_exp/99.0)*md->level) * per * bonus/100. * map[m].jexp/100., 1, UINT_MAX);
...
int mob_summonslave(struct mob_data *md2,int *value,int amount,int skill_id)
{
	...
	data.x = md2->bl.x;
	data.y = md2->bl.y;
	data.minlevel = (md2->level > 10) ? md2->level-10 : md2->level;
	data.maxlevel = (md2->level > 5) ? md2->level-5 : md2->level;
	data.num = 1;
...
static bool mob_parse_dbrow(char** str)
{
	...
	status->def = status->def3 = atoi(str[12]);
	status->mdef = status->mdef3 = atoi(str[13]);
