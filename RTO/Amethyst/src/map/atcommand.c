...
int atcommand_monster(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (sscanf(message, "\"%23[^\"]\" %23s %d %d %d", name, monster, &number, &minlv, &maxlv) > 1 ||
		sscanf(message, "%23s \"%23[^\"]\" %d %d %d", monster, name, &number, &minlv, &maxlv) > 1) {
		//All data can be left as it is.
	} else if ((count=sscanf(message, "%23s %d %23s %d %d", monster, &number, name, &minlv, &maxlv)) > 1) {
		//Here, it is possible name was not given and we are using monster for it.
		if (count < 3) //Blank mob's name.
			name[0] = '\0';
	} else if (sscanf(message, "%23s %23s %d %d %d", name, monster, &number, &minlv, &maxlv) > 1) {
		//All data can be left as it is.
	} else if (sscanf(message, "%23s %d %d", monster, &minlv, &maxlv) > 0) {
	...
	for (i = 0; i < number; i++) {
		...
		k = mob_once_spawn(sd, sd->bl.m, mx, my, name, mob_id, 1, "", minlv, maxlv);
		count += (k != 0) ? 1 : 0;
	}
...
int atcommand_monstersmall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (sscanf(message, "\"%23[^\"]\" %23s %d %d %d %d %d", name, monster, &number, &minlv, &maxlv, &x, &y) < 2 &&
	    sscanf(message, "%23s \"%23[^\"]\" %d %d %d %d %d", monster, name, &number, &minlv, &maxlv, &x, &y) < 2 &&
	    sscanf(message, "%23s %d %23s %d %d %d %d", monster, &number, name, &minlv, &maxlv, &x, &y) < 1) {
		clif_displaymessage(fd, "Give a monster name/id please.");
		return -1;
	}
	...
	for (i = 0; i < number; i++) {
		...
		if (y <= 0)
			...
			my = y;
		count += (mob_once_spawn(sd, sd->bl.m, mx, my, name, mob_id, 1, "2", minlv, maxlv) != 0) ? 1 : 0;
	}
...
int atcommand_monsterbig(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	for (i = 0; i < number; i++) {
		...
		if (y <= 0)
			...
			my = y;
		count += (mob_once_spawn(sd, sd->bl.m, mx, my, name, mob_id, 1, "4", minlv, maxlv) != 0) ? 1 : 0;
	}
...
int atcommand_recall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if((map[pl_sd->bl.m].name == "serverlobby" || map[pl_sd->bl.m].name == "pvprealm01" || map[pl_sd->bl.m].name == "pvprealm02" || map[pl_sd->bl.m].name == "pvprealm03" || map[pl_sd->bl.m].name == "pvprealm04" || map[pl_sd->bl.m].name == "pvprealm05" || map[pl_sd->bl.m].name == "pvprealm06" || map[pl_sd->bl.m].name == "pvprealm07" || map[pl_sd->bl.m].name == "pvprealm08" || map[pl_sd->bl.m].name == "pvprealm09" || map[pl_sd->bl.m].name == "pvprealm10" || map[pl_sd->bl.m].name == "pvprealm11" || map[pl_sd->bl.m].name == "pvprealm12" || map[pl_sd->bl.m].name == "pvprealm13" || map[pl_sd->bl.m].name == "pvprealm14" || map[pl_sd->bl.m].name == "pvprealm15" || map[pl_sd->bl.m].name == "pvprealm16" || map[pl_sd->bl.m].name == "pvprealm17" || map[pl_sd->bl.m].name == "pvprealm18" || map[pl_sd->bl.m].name == "pvprealm19" || map[pl_sd->bl.m].name == "pvprealm20") && pl_sd->bl.m != sd->bl.m) {
		clif_displaymessage(fd, "You can't warp across realms.");
	}
 ...
 * originally by [Lupus] eAthena
 *------------------------------------------*/
int atcommand_mdrops(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	char atcmd_output2[200];
	struct item_data *item_data;
	struct mob_db *mob, *mob_array[MAX_SEARCH];
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Please, enter a Monster/ID (usage: @mdrops <monster_name_or_monster_ID>).");
		return -1;
	}
...
int atcommand_togglepk(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	if (map[sd->bl.m].flag.gvg) {
		clif_displaymessage(fd, "You can't use PK mode on GvG maps.");
		if(sd->pk)
			sd->pk = 0;
		return 0;
	}
	...
	if(sd->pk) {
	  if(pk_checktime(sd) < battle_config.duel_time_interval) {
  		clif_displaymessage(fd, "You can only deactivate PK after 5 minutes of no action.");
  		sprintf(atcmd_output, "%d minutes have passed without action.", pk_checktime(sd));
  		clif_displaymessage(fd, atcmd_output);
  		return 0;
  	}
    sd->pk = 0;
    clif_set0199(sd, 0);
    clif_displaymessage(fd, "PK mode disabled.");
  } else {
		sd->pk = 1;
		clif_displaymessage(fd, "PK mode enabled.");
		pk_savetime(sd);
		clif_set0199(sd, 1);
...
AtCommandInfo atcommand_info[] = {
	...
	{ "b",				   60,	   atcommand_broadcast },
	{ "lb",					1,	   atcommand_localbroadcast },
	{ "shout",				1,     atcommand_localbroadcast },
	{ "pk",					1,	   atcommand_togglepk },
};
