...
int status_calc_mob(struct mob_data* md, int first)
{
	...
	{	//Set basic level on respawn.
		if(md->maxlevel && md->minlevel)
			md->level = rand()%(md->maxlevel-md->minlevel+1) + md->minlevel;
		else
			md->level = md->db->lv;
		/*if (md->spawn && !(first&2))
	...
	status->max_hp = status->hp = (unsigned int)(((float)status->hp/99.0) * (float)md->level);
	status->max_sp = status->sp = (unsigned int)(((float)status->sp/99.0) * (float)md->level);
	status->rhw.atk = (int)(((float)status->rhw.atk/99.0) * (float)md->level);
	status->rhw.atk2 = (int)(((float)status->rhw.atk2/99.0) * (float)md->level);
		
	int def = (int)(((float)status->def3/99.0) * (float)md->level);
	int mdef = (int)(((float)status->mdef3/99.0) * (float)md->level);

	status->def = (signed char)cap_value(def, CHAR_MIN, CHAR_MAX);
	status->mdef = (signed char)cap_value(mdef, CHAR_MIN, CHAR_MAX);

	status->str = (unsigned short)(((float)status->str/99.0) * (float)md->level);
	status->agi = (unsigned short)(((float)status->agi/99.0) * (float)md->level);
	status->vit = (unsigned short)(((float)status->vit/99.0) * (float)md->level);
	status->int_ = (unsigned short)(((float)status->int_/99.0) * (float)md->level);
	status->dex = (unsigned short)(((float)status->dex/99.0) * (float)md->level);
	status->luk = (unsigned short)(((float)status->luk/99.0) * (float)md->level);
	status_calc_misc(&md->bl, status, md->level);
	if(status->def+status->def2 > 100) {
		int tempDiff;
		tempDiff = (status->def+status->def2)-99;
		status->def -= tempDiff/2;
		status->def2 -= tempDiff/2;
	}
...
// ----- HIT CALCULATION -----
	...
	if((skill=pc_checkskill(sd,AC_VULTURE))>0){
		status->hit += skill;
		if(sd->status.weapon == W_BOW || (sd->status.weapon >= W_REVOLVER && sd->status.weapon <= W_GRENADE && sd->status.weapon != W_SHOTGUN))
...
static signed short status_calc_flee(struct block_list *bl, struct status_change *sc, int flee)
{
	...
	if(sc->data[SC_VIOLENTGALE])
		flee *= sc->data[SC_VIOLENTGALE]->val2/100;
...
unsigned char status_calc_attack_element(struct block_list *bl, struct status_change *sc, int element)
{
	...
	if(sc->data[SC_WINDWEAPON])
		return ELE_WIND;
	if( sc->data[SC_ENCPOISON] || sc->data[SC_EDP])
		return ELE_POISON;
...
int status_change_start(struct block_list* bl,enum sc_type type,int rate,int val1,int val2,int val3,int val4,int tick,int flag)
{
	...
	switch(type)
	{
		...
		case SC_EDP:	// [Celest]
			...
			val3 = 30*(val1+1); //Damage increase (+50 +50*lv%)
			break;
		case SC_POISONREACT:
