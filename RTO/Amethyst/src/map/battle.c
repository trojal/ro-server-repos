			...
			switch( skill_num )
			{
				...
				case MO_EXTREMITYFIST:
					{	//Overflow check. [Skotlex]
						unsigned int ratio = skillratio + (sstatus->sp/10+8) * 100 / status_get_lv(target) * status_get_lv(src);
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	{	//Checks on source master
		case BL_PC:
		{
			...
			if (((TBL_PC*)t_bl)->pk && t_bl->type == BL_PC && !((!battle_config.duel_allow_pvp && map[m].flag.pvp) || (!battle_config.duel_allow_gvg && map_flag_gvg(m))) ) {
					return (BCT_ENEMY&flag)?1:-1;
			}
