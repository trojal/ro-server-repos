 ...
 * PK Time Functions
 *------------------------------------------*/
void pk_savetime(struct map_session_data* sd)
{
	time_t timer;
	struct tm *t;
	...
	pc_setglobalreg(sd, "PC_LAST_PK_KILL_TIME", t->tm_mday*24*60 + t->tm_hour*60 + t->tm_min);	
	return;
}

int pk_checktime(struct map_session_data* sd)
{
	int diff;
	time_t timer;
	struct tm *t;
	...
	diff = t->tm_mday*24*60 + t->tm_hour*60 + t->tm_min - pc_readglobalreg(sd, "PC_LAST_PK_KILL_TIME");
