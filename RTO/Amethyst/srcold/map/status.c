...
//	StatusIconChangeTable[SC_DPOISON] =   SI_BLANK;
	...
	set_sc(BD_ETERNALCHAOS, SC_ETERNALCHAOS, SI_BLANK, SCB_DEF|SCB_MDEF|SCB_FLEE);
	set_sc(BD_DRUMBATTLEFIELD, SC_DRUMBATTLE, SI_BLANK, SCB_BATK|SCB_MATK|SCB_HIT);
	set_sc(BD_RINGNIBELUNGEN, SC_NIBELUNGEN, SI_BLANK, SCB_BATK);
	add_sc(BD_ROKISWEIL, SC_SILENCE);
	add_sc(BD_INTOABYSS, SC_INTOABYSS);
	set_sc(BD_SIEGFRIED, SC_SIEGFRIED, SI_BLANK, SCB_PC|SCB_DEF2|SCB_MDEF2);
	add_sc(BD_LULLABY, SC_SLEEP);
	add_sc(BA_FROSTJOKE, SC_FREEZE);
	set_sc(BA_WHISTLE, SC_WHISTLE, SI_BLANK, SCB_FLEE|SCB_HIT);
	set_sc(BA_ASSASSINCROSS, SC_ASSNCROS, SI_BLANK, SCB_ASPD|SCB_HIT|SCB_CRI);
	add_sc(BA_POEMBRAGI, SC_POEMBRAGI); //in skill.c:9037
	set_sc(BA_APPLEIDUN, SC_APPLEIDUN, SI_BLANK, SCB_MAXHP|SCB_REGEN);
	add_sc(DC_SCREAM, SC_STUN);
	set_sc(DC_HUMMING, SC_HUMMING, SI_BLANK, SCB_HIT|SCB_DEF|SCB_MDEF);
	set_sc(DC_DONTFORGETME, SC_DONTFORGETME, SI_BLANK, SCB_SPEED);
	set_sc(DC_FORTUNEKISS, SC_FORTUNE, SI_BLANK, SCB_LUK|SCB_FLEE2);
	set_sc(DC_SERVICEFORYOU, SC_SERVICE4U, SI_BLANK, SCB_MAXSP|SCB_REGEN);
	...
	set_sc(ASC_EDP, SC_EDP, SI_EDP, SCB_ATK_ELE);
...
int status_base_amotion_pc(struct map_session_data* sd, struct status_data* status)
{
	...
	// percentual delay reduction from stats based on weapon type
    switch(sd->status.weapon)
    {
		...
		case W_MACE:
        case W_GRENADE:
        	amotion-= amotion * (3*status->agi + status->str + status->dex)/1000;
            break;
        // 2 agi, 2 str, 1 dex
		case W_2HSWORD:
		case W_1HAXE:
        case W_SHOTGUN:
        	amotion-= amotion * (2*status->agi + 2*status->str + status->dex)/1000;
            break;
        // 1 agi, 3 str, 1 dex
		case W_2HAXE:
		case W_2HSPEAR:
        	amotion-= amotion * (status->agi + 3*status->str + status->dex)/1000;
            break;
        // 3 agi, 2 dex
		case W_STAFF:
		case W_BOW:
        case W_RIFLE:
        	amotion-= amotion * (3*status->agi + 2*status->dex)/1000;
            break;
        // 2 agi, 3 dex
		case W_MUSICAL:
		case W_WHIP:
        case W_REVOLVER:
        	amotion-= amotion * (2*status->agi + 3*status->dex)/1000;
            break;
        // 4 agi, 1 str
		case W_KNUCKLE:
		case W_KATAR:
		case W_BOOK:
        	amotion-= amotion * (4*status->agi + status->str)/1000;
            break;
        // gatling gun is unique: always fast, never changes with stats
        case W_GATLING:
        	amotion-= 200;
            break;
        // default: 4 agi, 1 dex
...
static int status_base_atk(struct block_list *bl, struct status_data *status)
{
	...
	if (bl->type == BL_PC)
		//str += 10 + (dstr*10)*2 + ((TBL_PC*)bl)->status.base_level/2 + dex/3 + status->luk/5;
...
void status_calc_misc(struct block_list *bl, struct status_data *status, int level)
{
	...
	status->def2 += status->vit*2 + level/2;
	status->mdef2 += status->int_ + (status->vit>>1);
...
int status_calc_pc(struct map_session_data* sd,int first)
{
	...
	sd->max_weight = max_weight_base[sd->status.class_]+sd->status.str*500;
...
// ----- HIT CALCULATION -----
	...
	if((skill=pc_checkskill(sd,AC_VULTURE))>0){
        ...
        (sd->status.weapon >= W_REVOLVER && sd->status.weapon <= W_GRENADE && sd->status.weapon != W_SHOTGUN))
    			status->rhw.range += skill;
	}
	if(sd->status.weapon >= W_REVOLVER && sd->status.weapon <= W_GRENADE && sd->status.weapon != W_GATLING)
  	{
		if((skill=pc_checkskill(sd,GS_SINGLEACTION))>0)
			status->hit += 2*skill;
		if((skill=pc_checkskill(sd,GS_SNAKEEYE))>0) {
			status->hit += skill;
			if(sd->status.weapon != W_SHOTGUN) status->rhw.range += skill;
		}
	}
...
// Unlike other stats, ASPD rate modifiers from skills/SCs/items/etc are first all added together, then the final modifier is applied
	...
	/* Figure out what amotion equals then modify it appropriately
	switch(sd->status.weapon) {
		case W_DAGGER:
			if(status->amotion < 100) status->amotion = 100;
			break;
		case W_1HSWORD:
			if(status->amotion < 120) status->amotion = 120;
			break;
		case W_2HSWORD:
			if(status->amotion < 140) status->amotion = 140;
			break;
		case W_1HSPEAR:
			if(status->amotion < 150) status->amotion = 150;
			break;
		case W_2HSPEAR:
			if(status->amotion < 160) status->amotion = 160;
			break;
		case W_1HAXE:
			//1ha
			if(status->amotion < 130) status->amotion = 130;
			break;
		case W_2HAXE:
			if(status->amotion < 150) status->amotion = 150;
			break;
		...
		case W_WHIP:
			...
			break;
		case W_KNUCKLE:
			if(status->amotion < 90) status->amotion = 90;
			break;
		case W_BOOK:
			if(status->amotion < 150) status->amotion = 150;
			break;
		case W_KATAR:
			if(status->amotion < 80) status->amotion = 80;
			break;
		case W_REVOLVER:
		case W_RIFLE:
		case W_SHOTGUN:
...
// ----- WEAPON HIT & ATK MODIFIERS -----

    // combat mastery check
    if((skill = pc_checkskill(sd,KN_SPEARMASTERY)) > 0)
    {
        status->batk *= (5*skill+100) / 100; // ATK + 5...50%
        status->hit += skill * 3; // HIT + 3...30pts
        status->cri += skill * 2; // CRI + 2...20pts
    }

	switch(sd->status.weapon) {
		
        // ABSOLUTELY DO NOT CHANGE THE -= DEDUCTORS HERE.

		case W_FIST:
			if((skill = pc_checkskill(sd,TK_RUN)) > 0) status->batk += skill * 10;
            // no extra change
			break;
		case W_DAGGER:
			if((skill = pc_checkskill(sd,AM_AXEMASTERY)) > 0) status->batk += skill*3; // 1-hand mastery
            if((skill = pc_checkskill(sd,SM_SWORD)) > 0) status->batk += skill*6; // weapon finesse
			status->batk *= 11 / 10; // ATK + 10%
			status->hit *= 6 / 5; // HIT + 20%
			status->cri *= 21 / 20; // CRI + 5%
			break;
		case W_1HSWORD:
            if((skill = pc_checkskill(sd,AM_AXEMASTERY)) > 0) status->batk += skill*3; // 1-hand mastery
            if((skill = pc_checkskill(sd,SM_SWORD)) > 0) status->batk += skill*6; // weapon finesse
			status->batk *= 4 / 3; // ATK + 33%
			status->hit *= 5 / 4; // HIT + 25%
			status->cri *= 11 / 10; // CRI + 10%
			break;
		case W_2HSWORD:
            if((skill = pc_checkskill(sd,SM_TWOHAND)) > 0) status->batk += skill*3; // 2-hand mastery
            if((skill = pc_checkskill(sd,PR_MACEMASTERY)) > 0) status->batk += skill*6; // power handling
			status->batk *= 8 / 5; // ATK + 60%
			status->hit *= 9 / 8; // HIT + 12.5%
			status->cri *= 11 / 10; // CRI + 10%
			break;
		case W_1HSPEAR:
            if((skill = pc_checkskill(sd,AM_AXEMASTERY)) > 0) status->batk += skill*3; // 1-hand mastery
            if((skill = pc_checkskill(sd,SM_SWORD)) > 0) status->batk += skill*6; // weapon finesse
			status->batk *= 7 / 4; // ATK + 40%
			status->hit *= 21 / 20; // HIT + 5%
			status->cri *= 11 / 10; // CRI + 10%
			break;
		case W_2HSPEAR:
            if((skill = pc_checkskill(sd,SM_TWOHAND)) > 0) status->batk += skill*3; // 2-hand mastery
            if((skill = pc_checkskill(sd,PR_MACEMASTERY)) > 0) status->batk += skill*6; // power handling
			status->batk *= 7 / 4; // ATK + 75%
			status->hit *= 11 / 10; // HIT + 10%
			status->cri *= 6 / 5; // CRI + 20%
			break;
		case W_1HAXE:
            if((skill = pc_checkskill(sd,AM_AXEMASTERY)) > 0) status->batk += skill*3; // 1-hand mastery
            if((skill = pc_checkskill(sd,SM_SWORD)) > 0) status->batk += skill*6; // weapon finesse
			status->batk *= 3 / 2; // ATK + 50%
			status->hit *= 9 / 8; // HIT + 12.5%
			status->cri *= 6 / 5; // CRI + 20%
			break;
		case W_2HAXE:
		...
		case W_MACE:
            if((skill = pc_checkskill(sd,AM_AXEMASTERY)) > 0) status->batk += skill*3; // 1-hand mastery
            if((skill = pc_checkskill(sd,SM_SWORD)) > 0) status->batk += skill*6; // weapon finesse
			status->batk *= 5 / 4; // ATK + 25%
			status->hit *= 11 / 10; // HIT + 10%
			status->cri *= 5 / 4; // CRI + 25%
			break;
        //view 9 is not used
		case W_STAFF:
            //  staff & rod weapons attack with autospell, using their MATK instead of ATK now.
            //  so they no longer crit, or do any physical damage.
			status->batk = 0;
			status->cri = 0;
			status->hit *= 21 / 20; // HIT + 5%
			break;
		case W_BOW:
			status->batk *= 5 / 4; // ATK + 25%
			status->hit -= status->hit / 5; // HIT - 20%
			status->cri *= 5 / 4; // CRI + 25%
            if((skill = pc_checkskill(sd,HT_POWER)) > 0) status->batk += skill*10;
			break;
		case W_KNUCKLE:
			if((skill = pc_checkskill(sd,AM_AXEMASTERY)) > 0) status->batk += skill*3; // 1-hand mastery
            if((skill = pc_checkskill(sd,MO_IRONHAND)) > 0) status->batk += skill*5;
			status->batk *= 11 / 10; // ATK + 10%
			status->hit *= 6 / 5; // HIT + 20%
			status->cri -= status->cri / 10; // CRI - 10%
			break;
		case W_MUSICAL:
			if((skill = pc_checkskill(sd,BA_MUSICALLESSON)) > 0) status->batk += skill*4;
            // no extra change
			break;
		case W_WHIP:
            if((skill = pc_checkskill(sd,DC_DANCINGLESSON)) > 0) status->batk += skill*4;
            // no extra change
			break;
		case W_BOOK:
            if((skill = pc_checkskill(sd,SA_ADVANCEDBOOK)) > 0) status->batk += skill*4;
			status->batk -= status->batk / 5; // ATK - 20%
			status->hit -= status->hit / 10; // ATK - 10%
			status->cri *= 11 / 10; // CRI + 10%
			break;
		case W_KATAR:
            if((skill = pc_checkskill(sd,AS_KATAR)) > 0) status->batk += skill*6;
			status->batk *= 11 / 10; // ATK + 10%
			status->hit *= 5 / 4; // HIT + 25%
			status->cri *= 11 / 10; // CRI + 10%
			break;
		case W_REVOLVER:
			status->batk *= 11 / 10; // ATK + 10%
			status->hit -= status->hit / 10; // HIT - 10%
			status->cri *= 6 / 5; // CRI + 20%
			break;
		case W_RIFLE:
			status->batk *= 5 / 4; // ATK + 25%
			status->hit *= 5 / 4; // HIT + 25%
			status->cri *= 13 / 10; // CRI + 30%
			break;
		case W_SHOTGUN:
			status->cri *= 7 / 5; // CRI + 40%
			break;
		case W_GATLING:
			status->hit *= 16 / 10; // HIT + 60%
			status->cri *= 11 / 10; // CRI + 10%
			break;
		case W_GRENADE:
			status->cri *= 13 / 10; // CRI + 30%
...
static unsigned short status_calc_batk(struct block_list *bl, struct status_change *sc, int batk)
{
	...
	if(sc->data[SC_NIBELUNGEN].timer!=-1)
		batk += sc->data[SC_NIBELUNGEN].val2;
		// if the target is an enemy of caster, val2 SHOULD be negative!
...
static signed short status_calc_critical(struct block_list *bl, struct status_change *sc, int critical)
{
	...
	if(sc->data[SC_ASSNCROS].timer!=-1)
		critical += sc->data[SC_ASSNCROS].val4;
...
static signed short status_calc_hit(struct block_list *bl, struct status_change *sc, int hit)
{
	...
	if(sc->data[SC_ASSNCROS].timer!=-1)
		hit += sc->data[SC_ASSNCROS].val2;
	if(sc->data[SC_DRUMBATTLE].timer!=-1)
		hit += sc->data[SC_DRUMBATTLE].val3;
	if(sc->data[SC_WHISTLE].timer!=-1 && sc->data[SC_WHISTLE].val1<0)
		hit += sc->data[SC_WHISTLE].val2; //only affects enemies; see skill.c
...
static signed short status_calc_flee(struct block_list *bl, struct status_change *sc, int flee)
{
	...
	if(sc->data[SC_ETERNALCHAOS].timer!=-1)
		flee -= sc->data[SC_ETERNALCHAOS].val4;
	if(sc->data[SC_WHISTLE].timer!=-1 && sc->data[SC_WHISTLE].val1>0)
		flee += sc->data[SC_WHISTLE].val2; //only affects allies; see skill.c
...
static signed char status_calc_def(struct block_list *bl, struct status_change *sc, int def)
{
	...
	if(sc->data[SC_ETERNALCHAOS].timer!=-1)
		def -= sc->data[SC_ETERNALCHAOS].val2;
	if(sc->data[SC_HUMMING].timer!=-1)
		def += sc->data[SC_HUMMING].val3;
...
static signed short status_calc_def2(struct block_list *bl, struct status_change *sc, int def2)
{
	...
	if(sc->data[SC_SIEGFRIED].timer!=-1)
		def2 += sc->data[SC_SIEGFRIED].val3;
...
static signed char status_calc_mdef(struct block_list *bl, struct status_change *sc, int mdef)
{
	...
	if(sc->data[SC_ETERNALCHAOS].timer!=-1)
		mdef -= sc->data[SC_ETERNALCHAOS].val3;
	if(sc->data[SC_HUMMING].timer!=-1)
		mdef += sc->data[SC_HUMMING].val4;
...
static signed short status_calc_mdef2(struct block_list *bl, struct status_change *sc, int mdef2)
{
	...
	if(sc->data[SC_SIEGFRIED].timer!=-1)
		mdef2 += sc->data[SC_SIEGFRIED].val3;
...
static unsigned short status_calc_speed(struct block_list *bl, struct status_change *sc, int speed)
{
	...
	if(sc->data[SC_DONTFORGETME].timer!=-1) {
		if(sc->data[SC_DONTFORGETME].val2>0) speed -= sc->data[SC_DONTFORGETME].val2; //bonus for allies
		else if(sc->data[SC_DONTFORGETME].val2<0) speed += sc->data[SC_DONTFORGETME].val2 * 2; //penalty for enemies
	}
...
int status_change_start(struct block_list *bl,int type,int rate,int val1,int val2,int val3,int val4,int tick,int flag)
{
	...
	case SC_FORTUNE:
	case SC_SERVICE4U:
		if(sc->data[SC_SLEEP].timer!=-1 && rand()%10<8) status_change_end(bl,SC_SLEEP,-1);
			else return 0; // sleeping has 80% immunity bardsong effects
		if(sc->data[SC_RICHMANKIM].timer!=-1 ) status_change_end(bl,SC_RICHMANKIM,-1);
		if(sc->data[SC_ETERNALCHAOS].timer!=-1 ) status_change_end(bl,SC_ETERNALCHAOS,-1);
		if(sc->data[SC_DRUMBATTLE].timer!=-1 ) status_change_end(bl,SC_DRUMBATTLE,-1);
		if(sc->data[SC_NIBELUNGEN].timer!=-1 ) status_change_end(bl,SC_NIBELUNGEN,-1);
		if(sc->data[SC_INTOABYSS].timer!=-1 ) status_change_end(bl,SC_INTOABYSS,-1);
		if(sc->data[SC_SIEGFRIED].timer!=-1 ) status_change_end(bl,SC_SIEGFRIED,-1);
		if(sc->data[SC_WHISTLE].timer!=-1 ) status_change_end(bl,SC_WHISTLE,-1);
		if(sc->data[SC_ASSNCROS].timer!=-1 ) status_change_end(bl,SC_ASSNCROS,-1);
		if(sc->data[SC_POEMBRAGI].timer!=-1 ) status_change_end(bl,SC_POEMBRAGI,-1);
		if(sc->data[SC_APPLEIDUN].timer!=-1 ) status_change_end(bl,SC_APPLEIDUN,-1);
		if(sc->data[SC_HUMMING].timer!=-1 ) status_change_end(bl,SC_HUMMING,-1);
		if(sc->data[SC_DONTFORGETME].timer!=-1 ) status_change_end(bl,SC_DONTFORGETME,-1);
		if(sc->data[SC_FORTUNE].timer!=-1 ) status_change_end(bl,SC_FORTUNE,-1);
		if(sc->data[SC_SERVICE4U].timer!=-1 ) status_change_end(bl,SC_SERVICE4U,-1);
		break;
	...
	switch(type){
		...
		// bardsongs
		case SC_RICHMANKIM:
			val2 = val1*10; // +10...50% EXP GAIN
			val3 = 50+val1*50; // +100...300z ZENY GAIN
			clif_specialeffect(bl, 268, AREA); //gold bag above head with flying gold coins
			break;
		case SC_ETERNALCHAOS:
			val2 = 10+rand()%41; // -10..50 DEF
			val3 = 10+rand()%41; // -10...50 MDEF
			val4 = 20+rand()%81; // -20...100 FLEE
			clif_specialeffect(bl, 54, AREA); //yellowish imploding cloud
			break;
		case SC_DRUMBATTLE:
			val2 = 40*val1; // +40...200 ATK/MATK
			val3 = 6*val1; // +6...30 HIT
			clif_specialeffect(bl, 538, AREA); //sprite flash green with green aura
			clif_specialeffect(bl, 491, AREA); //pointy yellow star-burst
			break;
		case SC_NIBELUNGEN:
			val2 = 20*val1; // +/-20...100 ATK/MATK
			if(val1>0) clif_specialeffect(bl, 392, AREA); //sword improvement effect?
			else if(val1<0) {
				clif_specialeffect(bl, 95, AREA); //quagmire bubbles
				clif_specialeffect(bl, 538, AREA); //sprite flashes green with green aura
			}
			break;
		case SC_INTOABYSS:
			clif_specialeffect(bl, 492, AREA); //white balls float to center, then rainbow-burst
			break;
		case SC_SIEGFRIED:
			val2 = 20*val1; // +20...100% Resist Status
			val3 = 16*val1; // +16...80 DEF2/MDEF2
			clif_specialeffect(bl, 144, AREA); //brandish spear pre-hit explosion
			clif_specialeffect(bl, 141, AREA); //pneuma monocell effect [skill must be 9 seconds long]
			break;
		case SC_WHISTLE:
			val2 = 3*val1; // +3...30 FLEE or -12...30 HIT
			if(val1>0) clif_specialeffect(bl, 273, AREA); //four white balls fly outwards, weird sound plays
			else if(val1<0) clif_specialeffect(bl, 234, AREA); //blue flash, white balls fly upwards
			break;
		case SC_ASSNCROS:
			val2 = 2*val1; // -2...20 ASPD [faster!]
			val3 = 4*val1; // +4...40 HIT
			val4 = 5*val1; // +5...50 CRI
			clif_specialeffect(bl, 348, AREA); //big blue spikey-star behind sprite
			clif_specialeffect(bl, 549, AREA); //sprite flashes blue
			break;
		case SC_POEMBRAGI:
			val2 = 3*val1; // +/- 3...30% CASTRATE
			val3 = 5*val1; // +/- 5...50% DELAYRATE
			if(val1>0) clif_specialeffect(bl, 72, AREA); //many white balls spiral downwards
			else if(val1<0) clif_specialeffect(bl, 71, AREA); //a few white balls spiral downwards
			break;
		case SC_APPLEIDUN:
			val2 = 2*val1; // +2...20% MAXHP
			sc_start4(bl, SC_REGENERATION,100,10,(val1*50),0,0,skill_get_time2(BA_APPLEIDUN, val1));
			// +50...500% HP NATURAL REGEN RATE
			clif_specialeffect(bl, 383, AREA); //many green balls fly outwards then back in
			break;
		case SC_HUMMING:
			val2 = 1+rand()%50; // +1-50? HIT
			val3 = 6^val1; // +6...30 DEF
			val4 = 10*val1; // +10...50 MDEF
			clif_specialeffect(bl, 402, AREA); //flash yellow with yellow spirals circling sprite
			break;
		case SC_DONTFORGETME:
			val2 = 5*val1; // -5...50 SPEED or +10...100 SPEED
			if(val1>0) clif_specialeffect(bl, 456, AREA); //agi-up without the text
			else if(val1<0) clif_specialeffect(bl, 38, AREA); //agi-down "slow" text
			break;
		case SC_FORTUNE:
			val2 = 5*val1; // +5...50 LUK
			val3 = val1; // +1...10 FLEE2
			clif_specialeffect(bl, 417, AREA); //sprite flashes white
			clif_specialeffect(bl, 153, AREA); //inc. concentration effect
			break;
		case SC_SERVICE4U:
			val2 = 4*val1; // +4...40% MAXSP
			sc_start4(bl, SC_REGENERATION,100,10,0,(val1*50),0,skill_get_time2(DC_SERVICEFORYOU, val1));
			// +50...500% SP NATURAL REGEN RATE
			clif_specialeffect(bl, 488, AREA); //tall warp-portal glow + chime sound
			break;
		...
		case SC_MAXIMIZEPOWER:
			...
			break;
		case SC_EDP:	// [Celest]
			val2 = val1 + 10; //Chance to Poison enemies.
