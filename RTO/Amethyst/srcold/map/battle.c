...
int battle_attr_fix(struct block_list *src, struct block_list *target, int damage,int atk_elem,int def_type, int def_lv)
{
	...
	if (sc && sc->count)
	{
		if(sc->data[SC_VOLCANO].timer!=-1 && atk_elem == ELE_FIRE)
			ratio += enchant_eff[sc->data[SC_VOLCANO].val1-1]*2;
		if(sc->data[SC_VIOLENTGALE].timer!=-1 && atk_elem == ELE_WIND)
			ratio += enchant_eff[sc->data[SC_VIOLENTGALE].val1-1]*2;
		if(sc->data[SC_DELUGE].timer!=-1 && atk_elem == ELE_WATER)
			ratio += enchant_eff[sc->data[SC_DELUGE].val1-1]*2;
	}
	if (tsc && tsc->count)
	{
...
int battle_addmastery(struct map_session_data *sd,struct block_list *target,int dmg,int type)
{
	...
	switch(weapon) // 1-handed mastery
	{
		case W_DAGGER:
		case W_1HSWORD:
		case W_1HSPEAR:
		...
		if((skill = pc_checkskill(sd,AM_AXEMASTERY)) > 0)
			damage += sd->battle_status.batk*skill/40;
		break;
    	}
    	switch(weapon) // 2-handed mastery
	{
		case W_2HSWORD:
		case W_2HAXE:
		case W_2HSPEAR:
	...
	switch(weapon) // Combat Mastery
	{
		...
		if((skill = pc_checkskill(sd,KN_SPEARMASTERY)) > 0)
			damage += sd->battle_status.batk*skill/20;
		break;
	}        
	switch(weapon) // Weapon Finesse
	{
		case W_DAGGER:
		case W_1HSWORD:
		case W_STAFF:
		...
		case W_2HSPEAR:
			if((skill = pc_checkskill(sd,KN_SPEARMASTERY)) > 0) {
				if(!pc_isriding(sd))
					...
					damage += sd->battle_status.batk*skill/30;
					//damage += (skill * 5);
			}
			break;
		case W_1HAXE:
	...
	struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int wflag)
{
	...
	{	//Hit/Flee calculation
        ...
        // PC projectile distance penalties
        if(sd)
        {
		...
		if (!skill_num)
		{
			...
			switch( skill_num )
			{
				...
				case MC_MAMMONITE:
					...
					break;
				case AC_DOUBLE:
					skillratio -= (skill_lv+1)/2 * 4;
                    if (pc_checkskill(sd, HT_POWER)) skillratio += sstatus->str/5;
                    // HT_POWER adds up to 25% damage this way
					break;
				case AC_SHOWER:
				...
				case HT_FREEZINGTRAP:
					skillratio += sstatus->int_*skill_lv;
					break;
				case KN_PIERCE:
				...
				case NPC_PULSESTRIKE:
                    if(sd) ATK_ADD(50+rand()%50);
					skillratio += 100*(skill_lv-1);
					break;
				case RG_BACKSTAP:
					skillratio += 100*skill_lv + (status_get_lv(src) - status_get_lv(target))*10;
					break;
				case RG_RAID:
				...
				case MO_EXTREMITYFIST:
					skillratio += ( sstatus->sp/10+8) * 100 / status_get_lv(target) * status_get_lv(src);
...
#define MATK_ADD( a ) { ad.damage+= a; }
		...
		{	//Calc base damage according to skill
			...
			case PR_TURNUNDEAD:
				...
				break;
			case PF_SOULBURN:
				ad.damage = tstatus->sp * 2 + sstatus->sp;
				break;
            case NPC_EARTHQUAKE:
                ad.damage = 2000 * skill_lv;
                break;
			default:
			{
				if (sstatus->matk_max > sstatus->matk_min) {
				...
				switch(skill_num){
					case MG_NAPALMBEAT:
                     	skillratio += 5*skill_lv+50;
                        skillratio -= status_get_lv(src);
			            break;
					case MG_FIREBALL:
						skillratio += 40*skill_lv;
                        skillratio -= status_get_lv(src);
					...
					case MG_SOULSTRIKE:
						skillratio += 5*skill_lv;
                        skillratio -= status_get_lv(src)/2;
						//	do extra if undead
					...
					case WZ_FIREPILLAR:
                        ...
                        skillratio -= status_get_lv(src)/10;
						break;
					case WZ_SIGHTRASHER:
						skillratio += 25*skill_lv;
                        skillratio -= status_get_lv(src);
                    ...
                    case WZ_METEOR:
                        skillratio += 50*skill_lv;
                        skillratio -= 2*status_get_lv(src);
                        break;
					case WZ_STORMGUST:
						skillratio += 15*skill_lv+50;
                        skillratio -= status_get_lv(src);
						break;
					...
					case SL_SMA:
						...
						break;
					case NJ_KOUENKA:
						skillratio += 20*skill_lv;//-10
						break;
					case NJ_KAENSIN:
						skillratio += 10*skill_lv;//-50
						break;
					case NJ_BAKUENRYU:
						skillratio += 100*(skill_lv-1);//50*
						break;
					case NJ_HYOUSYOURAKU:
						skillratio += 100*skill_lv;//50*
						break;
					case NJ_RAIGEKISAI:
						skillratio += 120 + 80*skill_lv;//60 and 40
						break;
					case NJ_KAMAITACHI:
	...
	struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int mflag)
{
	...
	switch(skill_num){
	case AS_GRIMTOOTH:
		md.damage = (sstatus->int_ + sstatus->dex) * skill_lv + 1000;
		break;
	case HT_LANDMINE:
		md.damage=skill_lv * sstatus->int_ * ( rand()%5 + 6);
		break;
	case HT_BLASTMINE:
		md.damage=skill_lv * sstatus->int_ * ( rand()%5 + 11);
		break;
	case HT_CLAYMORETRAP:
		md.damage=skill_lv * sstatus->int_ * ( rand()%10 + 16);
		break;
	case BA_DISSONANCE:
		md.damage=skill_lv * 300;
		break;
	case DC_UGLYDANCE:
		md.damage=skill_lv * 150;
		status_set_sp(target, (tstatus->sp-md.damage), 0);
		break;
	case HT_BLITZBEAT:
	case SN_FALCONASSAULT:
		...
		break;
	case TF_THROWSTONE:
		md.damage = 4*sstatus->str;
		//md.damage=50;
		break;
	case AS_SPLASHER:
        md.damage = (tstatus->max_hp - tstatus->hp) * skill_lv / 10;
		break;
	...
	case ASC_BREAKER:
		...
		break;
	case HW_GRAVITATION:
	    md.damage = ( status_get_lv(src)*3 - status_get_lv(target) ) * skill_lv;
	    if(md.damage < 100) md.damage = 100; //always does atleast 100 per
        if(md.damage > 4000) md.damage = 4000; //can't do more than 4000 per
...
// eAthena additions
	...
	{ "mob_bonus",				&battle_config.mob_bonus,			0,	0,	INT_MAX,	},
};
