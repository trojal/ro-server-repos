	...
	{	//Checks on source master
		case BL_PC:
		{
			...
			if( s_bl != t_bl )
			{
				...
				if (t_bl->type == BL_PC && map[m].flag.pvp && !map[m].flag.noracepk) 
		{ 
			if(sd->status.base_level >= battle_config.pk_min_level && ((TBL_PC*)t_bl)->status.base_level >= battle_config.pk_min_level) 
		{ 
		int race[2] = {-1, -1}; 
			int i = 1; 
				//Race Check 
				if( SQL_ERROR == Sql_Query(mmysql_handle, "SELECT `race` FROM `races` WHERE `char_id`='%d' OR `char_id`='%d' LIMIT 2", sd->status.char_id, ((TBL_PC*)t_bl)->status.char_id) ) 
				{ 
				...
				Sql_GetData(mmysql_handle, i, &race[i-1], NULL);
						i++; 
			} 
				if(race[0] != race[1] && race[0] != -1 && race[1] != -1) 
				return (BCT_ENEMY&flag)?1:-1; 
			} 
